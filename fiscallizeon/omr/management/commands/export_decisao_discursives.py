...
from fiscallizeon.omr.mockup_utils import print_mockup_discursive_sheet
...
from fiscallizeon.omr.tasks.tmp_group_discursive_sheet_files import group_discursive_sheet_files



class Command(BaseCommand):
    help = 'Gera malote com apenas folhas discusrivas para uma aplicação'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('delay_time', nargs='+', type=int)

    def handle(self, *args, **kwargs):
        export_version = kwargs['export_version'][0] or 90
        delay_time = kwargs['delay_time'][0] or 30
        ...
        for application_pk in self.APPLICATION_PKS:
            ...
            print_mockup_discursive_sheet(application, target_directory)
            ...
            for application_student in application_students_list:
                assets_tasks.append(
                    export_application_student_discursive_sheet.s(application_student['pk'], target_directory, blank_pages=False)
                )
            ...
            for school_class in school_classes:
                assets_tasks.append(
                    export_school_class_attendance_list.s(
                        application_pk, school_class.pk, target_directory, blank_pages=False
                    )
                )
            ...
            chord(
                ...
                group_discursive_sheet_files.si(
                    application_pk, export_version
                ).set(task_id=f'EXPORT_DISCURSIVES_{application_pk}_{export_version}')
            )
            time.sleep(delay_time)
            print(f'#### Tasks iniciada para aplicação {application_pk}')