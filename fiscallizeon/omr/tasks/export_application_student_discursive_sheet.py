...
def create_sheet_from_mockup(application_student, output_dir, omr_category_sequential, application_randomization_version=None):
    filename = f'mockup_discursive_sheet_{application_student.application_id}.html'
    if application_randomization_version:
        filename = f'mockup_discursive_sheet_{application_student.application_id}_v{application_randomization_version.sequential}.html'
    ...
    saved_file = fs.save(
        os.path.join(output_dir, f'discursive_{application_student.pk}.html'),
        open(tmp_file, 'rb')
    )
    os.remove(tmp_file)
...
def export_application_student_discursive_sheet(application_student_pk, output_dir, blank_pages=False, application_randomization_version_pk=None):
    try:
        print(f'Creating discursive answer sheet for {application_student_pk}')
        fs = PrivateMediaStorage()
        application_student = ApplicationStudent.objects.get(pk=application_student_pk)

        post_data = {
            "filename": f'discursive_{str(application_student_pk)}.pdf',
        ...
        post_data['url'] = create_sheet_from_mockup(application_student, output_dir, '8', application_randomization_version)
        return print_to_spaces(post_data)

    except Exception as e:
        print(f'### Error exporting discursive answer sheet for {application_student_pk}: {e}')