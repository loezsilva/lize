...
def import_students_task(self, user_id, csv_file_url, replace_old_classes=None):
    ...
    try:
        with transaction.atomic():
            ...
            for index, student in csv_content.iterrows():
                ...
                student_db = Student.objects.using('default').filter(
                    ...
                    Q(
                        Q(email__iexact=student['email'].strip())
                    )
                ).first()
                ...
                try:
                    aluno_nome = str(student_db.name)
                except:
                    pass

                if aluno_nome == "vazio":
                    aluno_nome += "ENTROU NO IF"
                    student_db = Student(
        ...
        if type(e) == SchoolCoordination.DoesNotExist:
            ...
            all_errors.append(f"Erro na linha {linha}: O aluno {student['nome']} com o e-mail {student['email']} já existe, altere o email do aluno e tente importar novamente. -{aluno_nome}- {e}")
            ...
            all_errors.append(f"Erro na linha {linha}: Ocorreu um erro desconhecido, entre em contato com o suporte para maiores informações {e}")
