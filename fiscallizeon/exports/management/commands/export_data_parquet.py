...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        print(f"### Iniciando exportação às {datetime.now()}")
        if client_pk := kwargs['client_pk']:
            client = Client.objects.filter(pk=client_pk)
        else:
        ...
        school_classes = SchoolClass.objects.filter(
            coordination__unity__client__in=client,
        ...
        inspectors = Inspector.objects.filter(
            coordinations__unity__client__in=client
        ).annotate(
        ...
        base_texts = BaseText.objects.filter(
            question__coordinations__unity__client__in=client,
        ).annotate(
        ...
        df = pd.DataFrame.from_records(exams, columns=[
            ...
            'correction_by_subject', 'elaboration_deadline', 'creator', 'release_elaboration_teacher', 'show_ranking'
        ])
        ...
        option_answers = OptionAnswer.objects.filter(
            student_application__student__client__in=client,
        ).annotate(
        ...
        df = pd.DataFrame.from_records(applications, columns=[
            'id', 'classes_ids', 'students_ids', 'exam_id', 'date', 'start', 'end', 'finish_date', 'category_description'
        ])
        df = df.astype({'id': str, 'exam_id': str})
        final_filename = os.path.join(self.base_tmp_dir, 'aplicacoes.parq')
        write(final_filename, df, compression='GZIP')

        print(f"### Exportação concluída às {datetime.now()}")