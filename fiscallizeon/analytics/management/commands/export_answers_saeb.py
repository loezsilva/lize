...
class Command(BaseCommand):
    help = 'Exporta todas as respostas de provas, calculando a TRI SAEB'

    DB_PATH = "tmp/tri-saeb.db"

    def add_arguments(self, parser):
        parser.add_argument('exams_uuids', nargs='+', type=str, help='Lista de IDs dos cadernos')
    ...
    def get_option_answers(self):
        subject_from_teacher_subject=Subquery(
        ...
        subject_from_question=Subquery(
            ExamQuestion.objects.filter(
                Q(exam=OuterRef('student_application__application__exam')),
                Q(question=OuterRef('question_option__question')),
            ).values(
        ...
        knowledge_area_from_teacher_subject=Subquery(
            ExamQuestion.objects.filter(
                Q(exam=OuterRef('student_application__application__exam')),
                Q(question=OuterRef('question_option__question')),
            ).values(
        ...
        knowledge_area_from_question=Subquery(
            ExamQuestion.objects.filter(
                Q(exam=OuterRef('student_application__application__exam')),
                Q(question=OuterRef('question_option__question')),
            ).values(
        ...
        textual_answers = OptionAnswer.objects.filter(
            ...
            subject_name=Case(
                When(student_application__application__exam__is_abstract=True, then=subject_from_question),
                When(student_application__application__exam__is_abstract=False, then=subject_from_teacher_subject),
                output_field=CharField(),
            ),
            knowledge_area_name=Case(
                When(student_application__application__exam__is_abstract=True, then=knowledge_area_from_question),
                When(student_application__application__exam__is_abstract=False, then=knowledge_area_from_teacher_subject),
                output_field=CharField(),
            ),
    ...
    def get_application_students(self):
        ...
        return application_students.values(
            ...
            'exam_id', 'school_class_name', 'grade_name',  
            'unity_name', 'is_attendant')
    ...
    def handle(self, *args, **kwargs):
        ...
        uuids = kwargs['exams_uuids']
        self.exams_pks = []

        for uuid_str in uuids:
            try:
                valid_uuid = uuid.UUID(uuid_str)
                self.exams_pks.append(uuid_str)
            except ValueError:
                self.stdout.write(self.style.ERROR(f'UUID inválido: {uuid_str}'))

        print(self.exams_pks)
        print(f'Processando respostas')
        ...
        for exam in self.exams_pks:
            ...
            if has_correct_answers and has_incorrect_answers:
                _tri_df = self.compute_tri([exam])
                _tri_df['exam_id'] = exam
