...
def generate_performance(self, recalculate=True):
    ...
    topics = Topic.objects.filter(question__examquestion__in=questions_availables, subject__isnull=False, subject__knowledge_area__grades__level=Grade.HIGHT_SCHOOL).distinct()
    abilities = Abiliity.objects.filter(question__examquestion__in=questions_availables, subject__isnull=False, subject__knowledge_area__grades__level=Grade.HIGHT_SCHOOL).distinct()
    competences = Competence.objects.filter(question__examquestion__in=questions_availables, subject__isnull=False, subject__knowledge_area__grades__level=Grade.HIGHT_SCHOOL).distinct()
