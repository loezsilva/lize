...
from fiscallizeon.bncc.serializers.competence import CompetenceSerializer, CompetenceViewSetSerializer, CompetenceCreateApiSerializer
...
class CompetenceRetrieveAPIView(LoginRequiredMixin, CheckHasPermission, RetrieveAPIView):
            ...
            if classes:
                for classe in classes:
                    data['classes'].append({
                        ...
                        "value": self.get_competence_summary(instance, exam, classe),
                    })
            ...
            if unities:
                for unity in unities:
                    data['unities'].append({
                        "id": unity.id,
                        "name": unity.name,
                        "value": self.get_competence_summary(instance, exam, None, unity),
                    })
        ...
        if school_class:
            applications_student = applications_student.annotate(
                ...
                all_answers=Count('option_answers', filter=Q(
                    Q(option_answers__status=OptionAnswer.ACTIVE, option_answers__student_application__student__classes=school_class, option_answers__question_option__question__competences=competence),
                    Q(
...
class CompetenceViewSet(LoginRequiredMixin, CheckHasPermission, ModelViewSet):
    queryset = Competence.objects.all()
    serializer_class = CompetenceViewSetSerializer
...
class CompetenceCreateApiView(LoginRequiredMixin, CheckHasPermission, CreateAPIView):
    queryset = Competence.objects.all()
    serializer_class = CompetenceCreateApiSerializer
    required_permissions = [settings.COORDINATION, settings.TEACHER]
...
competence_create_api = CompetenceCreateApiView.as_view()